...
1.  **편집 중 (Editing)**: 초기 상태. 사용자가 디자인을 자유롭게 편집하고 저장할 수 있습니다.
2.  **잠정 주문 (Tentative Order)**: 사용자가 편집을 마치고 주문을 요청한 상태. (편집 잠금)
3.  **확정 주문 (Definitive Order)**: 결제 완료 등으로 주문이 최종 승인된 상태. (생산 시작)

---

## 1.잠정 주문 (Tentative Order)
...
*   **전제 조건**: 주문 상태가 반드시 **잠정 주문 (Tentative Order)** 상태여야 합니다. (확정 주문은 취소 불가)
*   **결과**:
    *   주문 상태가 다시 **편집 중 (editing)**으로 변경됩니다.
